...
GART_fun <- function(data, tau){
  ...
  base_names = c('weighted', 'convex', 'source', 'zero')
  ...
  for(which_beta_to_use in c('target_only')){
    ...
    for(fold in c(0, 1)){
      ...
      for(base in base_names){
        ...
        problem = switch(base, 
                         ...
                         affine = Problem(objective, constraints =
                                                  list(gammaHat[1]>=0, gammaHat[2:(L+1)] >= -ini_bound, gammaHat <= ini_bound, 
                                                       sum(gammaHat) == 1)),
                         positive = Problem(objective, constraints =
                                                    list(gammaHat>=0, gammaHat <= ini_bound)),
    ...
    names(maximin_base_ave_later_lst) = paste0('GART_later_base_', base_names)
    ...
    if(which_beta_to_use == 'target_only'){
      beta_est = cbind(beta_est, cbind(sapply(maximin_base_ave_first_lst, function(x) x$beta_hat)))
      gamma_est = cbind(gamma_est, cbind( sapply(maximin_base_ave_first_lst, function(x) x$gamma_hat))) 
    }else{
      beta_est = cbind(beta_est, cbind(sapply(maximin_base_ave_first_lst, function(x) x$beta_hat)))
      gamma_est = cbind(gamma_est, cbind(sapply(maximin_base_ave_first_lst, function(x) x$gamma_hat)))
    }
  ...
  beta_est = cbind(`true_beta` = data$beta_valid,
                   beta_est,
...
evaluate_fun <- function(beta_valid, beta_est,
  ...
  predict_mse = as.vector(y_target_valid) - X_target_valid %*% as.matrix(beta_est)
  predict_mse = apply(predict_mse, 2, function(x){
    mean(x^2)
  })
...
Trans_label_est_split <- function(fold, tau,  X_source_matrix, base, B_source_hat, beta_target_hat,
  ...
  # print(paste0('s2_Q_hat_ini_target=', round(s2_Q_hat_ini_target, 3)))
  
  s2_Q_hat_ini_linear = ini_all$ini_linear_comb$s2_Q
  gamma_hat_ini_linear = ini_all$ini_linear_comb$gamma
  # print(paste0('s2_Q_hat_ini_linear=', round(s2_Q_hat_ini_linear, 3)))
  ...
  # s2_Q_hat_min = min(s2_Q_hat_ini_linear, s2_Q_hat_ini_target, s2_Q_hat_ini_robust)
  s2_Q_hat_min = min(s2_Q_hat_ini_linear, s2_Q_hat_ini_target) + tau
  # print(paste0('s2_Q_hat_min=', round(s2_Q_hat_min, 3)))
...
# 
# Trans_label_est_split_ave <-  function(X_source_matrix, B_source_hat, base,
#                                        beta_target_hat_1, beta_target_hat_2,
#                                        X_target, y_target, N_target, N_source, 
#                                        beta_hat_step1, beta_target_only, which_beta_to_use){
#   L = ncol(B_source_hat)
...
#   gamma_hat_ini_linear_robust = ini_all$ini_robust_linear_comb$gamma
#   
#   s2_Q_hat_min_1 = min(s2_Q_hat_ini_linear, s2_Q_hat_ini_target)
#   
#   ini_all = est_ini(fold = 0, sample = 'split', nfold = NA, X_target, y_target, 
#                     Sigma_Q_hat, Gamma_beta_hat, B_source_hat, beta_target_hat_1, L)
#   
...
#   s2_Q_hat_ini_linear_robust = ini_all$ini_robust_linear_comb$s2_Q
#   gamma_hat_ini_linear_robust = ini_all$ini_robust_linear_comb$gamma
#   s2_Q_hat_min_2 = min(s2_Q_hat_ini_linear, s2_Q_hat_ini_target)
#   
#   s2_Q_hat_min = mean(c(s2_Q_hat_min_1, s2_Q_hat_min_2))
#   
#   beta_hat_ini_target = beta_target_only 
#   beta_hat_ini_robust = beta_hat_step1
#   
#   coef_min_s2Q_robust = 
#     Trans_label_robust_A_robust(X_target, y_target, base,
#                                 B_source_hat, Sigma_Q_hat, beta_hat_ini_target,
#                                 beta_hat_ini_target,  s2_Q_hat_min, beta_hat_ini_robust)
#   
#   coef_min_s2Q_robust_A_robust = list(`gamma_hat` = coef_min_s2Q_robust$gamma_hat,
#                                       `beta_hat` = coef_min_s2Q_robust$beta_hat,
#                                       `success` = coef_min_s2Q_robust$success)
#   result = list(
#     `min_s2Q_robust_A_robust_split` = coef_min_s2Q_robust_A_robust,
#     `ini_split` = ini_all
#   )
...
Trans_label_robust_A_robust <- function(X_target_test, y_target_test, base,
  ...
  # singular_min = min(svd(Gamma_beta_hat_new)$d)
  # print(paste0('Min singular value for Gamma: ', round(singular_min, 4)))
  # if(singular_min <= 0){
  #   Gamma_beta_hat_new = Gamma_beta_hat_new + 0.001 * diag(1, nrow = nrow(Gamma_beta_hat_new))
  #   print('Eigen-adjusted!')
  # }
  ...
  #                  affine_bound = Problem(objective, constraints =
  #                                           list(constraints0, gammaHat[1]>=0, gammaHat[2:(L+1)] >= -ini_bound, gammaHat <= ini_bound, 
  #                                                sum(gammaHat) == 1)),
  #                  positive_bound = Problem(objective, constraints =
  #                                             list(constraints0, gammaHat>=0, gammaHat <= ini_bound)),
  #                  weighted_bound = Problem(objective, constraints =
  #                                             list(constraints0, gammaHat[1]>=0, gammaHat[2:(L+1)] >= -ini_bound, gammaHat <= ini_bound)),
  #                  zero = Problem(objective, constraints =
  #                                     list(constraints0, gammaHat >= 0, gammaHat <= 1, sum(gammaHat)==1))
  #                  
  ...
  if(result$status %in% c('optimal', 'optimal_inaccurate')){
    ...
    success = 1
    # print(paste0('Est gamma:', paste0(round(gamma_hat,4), collapse = ', ')))
  }else{
